...
{
    public partial interface IReportLocationService
    {
        Task GetPage(Pipeline pipeline, ReportLocationFilterVM filter);
    }

    public partial class ReportLocationService : _ServiceGpsTracking, IReportLocationService
    {
        public ReportLocationService(IServiceProvider _ServiceProvider)
            : base(_ServiceProvider)
        {
        }

        public async Task GetPage(Pipeline pipeline, ReportLocationFilterVM filter)
        {
            //Select
            var ReportLocations = DBs.Location.Query;
            ...
            if (lib.Selected(filter.GpsDeviceId))
            {
                ...
                if (gpsDevice != null)
                {
                    ...
                    ReportLocations = ReportLocations.Where(x => x.GpsDeviceId == filter.GpsDeviceId);
                }
            }
            ...
            if (lib.Selected(filter.GroupId))
            {
                ...
                if (group != null)
                {
                    ...
                    ReportLocations = ReportLocations.Where(x => group.GpsDeviceMappings.Any(y => y.GpsDeviceId == x.GpsDeviceId));
                }
            }
            ...
            //filter.Date
            if (filter.HaveDate())
                ReportLocations = ReportLocations.Where(x => x.CreatedTime >= filter.FromDate && x.CreatedTime <= filter.ToDate);

            //Order
            ReportLocations = ReportLocations.OrderByDescending(x => x.CreatedTime);

            //Models
            var Total = ReportLocations.Count();

            //ExportFile / Skip / Take
            if (filter.ExportFile == false)
                ReportLocations = ReportLocations.Skip(filter.From).Take(Config.MaxSelectPage(filter.From, filter.To));

            var Models = await ReportLocations
            ...
            {
                var export = new ReportExport(pipeline, filter, "Location", "Report-Location", false, 9, Models.Count - 1, 1, 7);
                ...
                foreach (var model in Models)
                {
                    ...
                    export.Sheet.SetExcelCell(export.CurrentRow, 4, model.Longitude);
                    export.Sheet.SetExcelCell(export.CurrentRow, 5, model.Latitude);
                    export.Sheet.SetExcelCell(export.CurrentRow, 6, model.Speed);
                    export.Sheet.SetExcelCell(export.CurrentRow, 7, model.Address);
                ...
                if (filter.SendEmail == true)
                {
                    ...
                    string subject = "Báo cáo lộ trình: " + lib.Time;
                    string content = "Báo cáo lộ trình của phần mềm GPS, tính đến thời điểm " + lib.Time;
